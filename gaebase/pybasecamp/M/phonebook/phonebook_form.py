...
def form_(self, record_key, action_):
  ...
  ret_val =  """<form action='/phonebook/M/""" + ((action_ + "/" + sanitize_txt(record_.key())) if (record_) else action_) +"""' method="post">
		      ...
		      <tr><td>Email#:</td><td><input name="email" id="email" type="text" value='""" + (sanitize_txt(record_.email) if (record_) else 'e@mail.id') + """' onfocus="this.value=''"></text></td></tr>
...
def static_form_(self, record_key):
  ...
  if(record_key):
    ...
    ret_val =  """<div>
		      ...
		      <tr><td>Email#:</td><td><label name="email" id="email">""" + (sanitize_txt(record_.email) if (record_) else 'e@mail.id') + """</label></td></tr>
...
def static_list_(self, action_):
  ...
  for record_ in datab_:
    ret_val = ret_val + "<tr>"
    ret_val = ret_val + "<td>" + sanitize_txt(record_.key().id()) + "</td>"
    ret_val = ret_val + "<td>" + sanitize_txt(record_.name) + "</td>"
    ret_val = ret_val + "<td>" + sanitize_txt(record_.phonenumber) + "</td>"
    ret_val = ret_val + "<td>" + sanitize_txt(record_.email) + "</td>"
    ret_val = ret_val + "<td>" + sanitize_txt(record_.description) + "</td>"
    ret_val = ret_val + "<td>" + sanitize_txt(record_.timestamp_) + "</td>"
    ret_val = ret_val + "<td><a id=\"update\" href=\"/phonebook/V/" + (action_ if(action_) else "read") + "/" + sanitize_txt(record_.key()) + "\">"
